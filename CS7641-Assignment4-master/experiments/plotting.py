...
def plot_episode_stats(title_base, stats, smoothing_window=50):
    ...
    plt.ylabel("Count")
    plt.tight_layout()
    plt.title(title_base.format("Episode Steps"))
    fig1 = watermark(fig1)
    ...
    plt.ylabel("Count")
    plt.title(title_base.format("Episode Reward"))
    fig2 = watermark(fig2)
...
def plot_reward_and_delta_vs_steps(title, df, xlabel="Steps", ylabel="Reward"):
    ...
    lns1 = ax.plot(df.index.values, df['reward'], linewidth=1, label="Reward", color="black")

    ex_ax = ax.twinx()
    lns2 = ex_ax.plot(df.index.values, df['delta'], linewidth=1, label='Convergence', color="red")
    ex_ax.set_ylabel('Convergence')
    ex_ax.tick_params('y')
...
def plot_data(data_files, envs, base_dir):
    for problem_name in data_files:
        for mdp in data_files[problem_name]:
            ...
            title = '{}: {} - {} and Convergence vs {}'.format(env['readable_name'],
