...
def AUC_score(Y, T):
    ...
    for j in range(101):
        ...
        TP_t = get_count([predicted[i] == 1.0 and x == 1.0 for i, x in enumerate(labels)])
        TN_t = get_count([predicted[i] == 0.0 and x == 0.0 for i, x in enumerate(labels)])
...
def error_on_data_set(network, ds, measure, ugh=False):
    ...
    for instance in ds:
        ...
        predicteds_raw.append(predicted)
    ...
    auc_score = AUC_score(actuals, predicteds_raw)
    if ugh:
        print "MSE: {}, acc: {}, AUC: {}".format(MSE, acc, auc_score)
        import sys
        sys.exit(0)

    return MSE, acc, auc_score 
